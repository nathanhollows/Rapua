...
- Refactored the `GameManagerService` into `GameScheduleService` and `QuickstartService` to improve maintainability and separation of concerns.
- Refactored the `LocationService` into `LocationStatsService`.

...
func (s *NavigationService) GetPlayerNavigationView( //nolint:gocognit,gocyclo
	...
	if currentGroup != nil && currentGroup.Navigation == models.NavigationTasks {
		uncompleted, completed, err := s.getScavengerHuntLocations(ctx, navTeam, currentGroup)
		if err != nil {
			return nil, fmt.Errorf("loading scavenger hunt locations: %w", err)
		}
		uncompleted = filterLocationsByWhen(uncompleted, resolver)
		completed = filterLocationsByWhen(completed, resolver)
	...
	locations, err := s.determineNextLocations(ctx, navTeam)
	if err != nil {
		return nil, err
	}

	...
	if r.Form.Get("set_start") != "" {
		...
		if startDate == "" || startTime == "" {
			h.handleError(
				w,
				r,
				"ScheduleGame: missing start date or time",
				"Error parsing start date and time",
		...
		if err != nil {
			h.handleError(
				w,
				r,
				"ScheduleGame: parsing start date and time",
				"Error parsing start date and time",
	...
	if r.Form.Get("set_end") != "" {
		...
		if endDate == "" || endTime == "" {
			h.handleError(
				w,
				r,
				"ScheduleGame: missing end date or time",
				"Error parsing end date and time",
		...
		if err != nil {
			h.handleError(
				w,
				r,
				"ScheduleGame: parsing end date and time",
				"Error parsing end date and time",
				"Could not parse date and time",
				err,
				"instance_id",

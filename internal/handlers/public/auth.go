...
func (h *Handler) ForgotPasswordPost(w http.ResponseWriter, r *http.Request) {
	...
	if emailErr := h.emailService.SendPasswordResetEmail(r.Context(), *user, resetURL); emailErr != nil {
		h.logger.Warn("ForgotPasswordPost: sending email failed", "error", emailErr)
	}
